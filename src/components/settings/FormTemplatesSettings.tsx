...
import { 
  ...
  FileText, Type, AlignLeft, ListChecks, ToggleLeft, Upload, Copy, Loader2
} from "lucide-react";
...
export default function FormTemplatesSettings() {
  ...
  const [formData, setFormData] = useState<{
    name: string; description: string; isDefault: boolean; fields: FormField[];
  }>({ name: "", description: "", isDefault: false, fields: [] });

  const filteredTemplates = templates.filter(t => showArchived ? t.isArchived : !t.isArchived);

  const openCreateDialog = () => {
    setEditingTemplateId(null);
    setFormData({ name: "", description: "", isDefault: false, fields: [] });
  ...
  const openEditDialog = (templateId: string) => {
    ...
    setFormData({ name: template.name, description: template.description || "", isDefault: template.isDefault, fields: [...template.fields] });
    setIsDialogOpen(true);
  };

  const addField = () => {
    const newField: FormField = { id: Date.now().toString(), label: "", type: "short_text", required: false, order: formData.fields.length + 1 };
    setFormData({ ...formData, fields: [...formData.fields, newField] });
  };

  const updateField = (fieldId: string, updates: Partial<FormField>) => {
    setFormData({ ...formData, fields: formData.fields.map(f => f.id === fieldId ? { ...f, ...updates } : f) });
  };
  ...
  const handleSave = async () => {
    ...
    if (formData.fields.length === 0) { toast.error("Adicione pelo menos um campo"); return; }
    if (formData.fields.some(f => !f.label.trim())) { toast.error("Todos os campos precisam de um rótulo"); return; }
  ...
  if (isLoading) {
    return (
      <Card>
        <CardHeader><CardTitle>Templates de Formulário</CardTitle><CardDescription>Carregando...</CardDescription></CardHeader>
        <CardContent className="space-y-3">{[1, 2, 3].map(i => <Skeleton key={i} className="h-16 w-full" />)}</CardContent>
      </Card>
    );
  }
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            ...
            <CardDescription>Crie formulários reutilizáveis para as vagas</CardDescription>
          </div>
        ...
        <CardContent>
          {filteredTemplates.length === 0 ? (
            <div className="flex flex-col items-center justify-center rounded-lg border border-dashed py-12">
              ...
              <p className="text-muted-foreground text-center max-w-sm">Crie templates de formulário para coletar informações dos candidatos</p>
                ...
                <div key={template.id} className={`flex items-center justify-between rounded-lg border p-4 transition-colors hover:bg-muted/50 ${template.isArchived ? 'opacity-60' : ''}`}>
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">{template.fields.length} campo(s){template.description && ` • ${template.description}`}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-1">
                    ...
                    <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => openEditDialog(template.id)}><Pencil className="h-4 w-4" /></Button>
                    {template.isArchived ? (
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>Configure os campos do formulário de candidatura</DialogDescription>
          </DialogHeader>
          <div className="space-y-6 py-4">
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-2">
                <Label htmlFor="name">Nome do Template</Label>
                <Input id="name" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} placeholder="Ex: Formulário Tech" />
              </div>
              <div className="space-y-2">
                <Label htmlFor="description">Descrição</Label>
                <Input id="description" value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} placeholder="Descrição opcional" />
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <Button type="button" variant="outline" size="sm" onClick={addField}><Plus className="h-4 w-4 mr-1" />Adicionar Campo</Button>
              </div>
              {formData.fields.length === 0 ? (
                <div className="text-center py-8 border border-dashed rounded-lg">
                  <p className="text-muted-foreground">Nenhum campo adicionado</p>
                  <Button type="button" variant="link" onClick={addField}>Adicionar primeiro campo</Button>
                </div>
              ) : (
                <div className="space-y-3">
                  {formData.fields.map((field) => (
                    <div key={field.id} className="flex items-start gap-3 rounded-lg border p-3 bg-muted/30">
                      <div className="pt-2"><GripVertical className="h-4 w-4 text-muted-foreground cursor-grab" /></div>
                      <div className="flex-1 grid gap-3 sm:grid-cols-[1fr,160px,auto]">
                        <Input value={field.label} onChange={(e) => updateField(field.id, { label: e.target.value })} placeholder="Rótulo do campo" />
                        <Select value={field.type} onValueChange={(value: FormFieldType) => updateField(field.id, { type: value })}>
                          <SelectTrigger><SelectValue /></SelectTrigger>
                          <SelectContent>
                            {Object.entries(fieldTypeConfig).map(([type, config]) => (
                              <SelectItem key={type} value={type}><div className="flex items-center gap-2">{config.icon}<span>{config.label}</span></div></SelectItem>
                            ))}
                          </SelectContent>
                        </Select>
                        <div className="flex items-center gap-2">
                          ...
                          <Button type="button" variant="ghost" size="icon" className="h-8 w-8 text-destructive" onClick={() => removeField(field.id)}><Trash2 className="h-4 w-4" /></Button>
                        </div>
                      </div>
                    </div>
                  ))}
